...
export const VendorSidebar = () => {
  ...
  const { vendor, user, logout } = useVendorAuth();
  const [expandedMenus, setExpandedMenus] = useState<string[]>([]);

  // Load expanded menus from localStorage on mount
  useEffect(() => {
    const savedExpandedMenus = localStorage.getItem('vendor-sidebar-expanded-menus');
  ...
  const userInitials = userName?.split(' ')?.map(n => n?.[0] || '').filter(Boolean).join('').toUpperCase().slice(0, 2) || 'VU';
  ...
  const toggleMenu = (title: string) => {
    setExpandedMenus((prev) => {
      ...
      localStorage.setItem('vendor-sidebar-expanded-menus', JSON.stringify(newExpanded));
      return newExpanded;
    });
  };
