...
class TenantApiClient {
  ...
  constructor() {
    ...
    console.log('[TenantApiClient v2.0 - API FIX APPLIED] BaseURL:', this.baseURL);
  ...
  private setupInterceptors(): void {
    ...
    this.client.interceptors.request.use(
      (config) => {
        ...
        // Normalize URL - remove /api/v1 or /tenant prefix if present (v2.0 FIX)
        if (config.url && !config.url.startsWith('http')) {
          const originalUrl = config.url;
          let url = config.url;
          
          // Remove /api/v1/tenant prefix if present
          if (url.startsWith('/api/v1/tenant/')) {
            url = url.substring(15);
            console.log(`[TenantApiClient v2.0] ✅ Stripped /api/v1/tenant: ${originalUrl} → ${url}`);
          } else if (url.startsWith('/api/v1/')) {
            url = url.substring(7);
            console.log(`[TenantApiClient v2.0] ✅ Stripped /api/v1: ${originalUrl} → ${url}`);
          } else if (url.startsWith('/tenant/')) {
            url = url.substring(8);
            console.log(`[TenantApiClient v2.0] ✅ Stripped /tenant: ${originalUrl} → ${url}`);
          }
