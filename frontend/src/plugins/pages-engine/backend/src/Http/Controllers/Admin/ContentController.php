...
use Plugins\PagesEngine\Http\Requests\UpdateContentRequest;
...
class ContentController extends Controller
{
    ...
    public function store(CreateContentRequest $request): JsonResponse
    {
        ...
        $command = new CreateContentCommand(
            ...
            authorId: new \Plugins\PagesEngine\Domain\ValueObjects\Uuid(auth()->user()->uuid),
        ...
        $contentModel = \Plugins\PagesEngine\Infrastructure\Persistence\Eloquent\ContentEloquentModel::where('uuid', $content->uuid)
            ->where('tenant_id', auth()->user()->tenant_id)
            ->first();
        if ($contentModel) {
    ...
    public function update(UpdateContentRequest $request, string $uuid): JsonResponse
    {
        ...
        $contentModel = \Plugins\PagesEngine\Infrastructure\Persistence\Eloquent\ContentEloquentModel::where('uuid', $uuid)
            ->where('tenant_id', auth()->user()->tenant_id)
            ->first();
        if ($contentModel) {
            if ($request->has('categories')) {
    ...
    public function destroy(string $uuid): JsonResponse
    {
        ...
        $command = new \Plugins\PagesEngine\Application\Commands\DeleteContentCommand(
            contentId: new \Plugins\PagesEngine\Domain\ValueObjects\Uuid($uuid),
            tenantId: new \Plugins\PagesEngine\Domain\ValueObjects\Uuid(auth()->user()->tenant->uuid)
        );
    ...
    public function schedule(Request $request, string $uuid): JsonResponse
    {
        ...
        $command = new ScheduleContentCommand(
            contentId: new \Plugins\PagesEngine\Domain\ValueObjects\Uuid($uuid),
            tenantId: new \Plugins\PagesEngine\Domain\ValueObjects\Uuid(auth()->user()->tenant->uuid),
            scheduledPublishAt: new \DateTime($request->input('scheduled_at'))
        );
