...
final class ContentCategoryEloquentRepository implements ContentCategoryRepositoryInterface
{
    ...
    {
        $eloquentModel = $this->model->with('tenant')->where('uuid', $id->getValue())->first();
    ...
    {
        $query = $this->model->with('tenant')->where('tenant_id', $tenantId);
    ...
    private function mapToEntity(ContentCategoryEloquentModel $model): ContentCategory
    {
        return new ContentCategory(
            ...
            tenantId: new Uuid($model->tenant->uuid),
    ...
    private function mapToArray(ContentCategory $category): array
    {
        return [
            ...
            'tenant_id' => \App\Infrastructure\Persistence\Eloquent\TenantEloquentModel::where('uuid', $category->getTenantId()->getValue())->value('id'),
