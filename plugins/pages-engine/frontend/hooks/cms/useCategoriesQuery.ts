...
export const useCategoryTreeQuery = (contentTypeUuid?: string) => {
  const { userType } = useGlobalContext();
  const { tenant, user } = useTenantAuth();

  return useQuery({
    queryKey: queryKeys.categories.tree(contentTypeUuid),
    queryFn: async (): Promise<ApiResponse<CategoryTreeNode[]>> => {
      ...
      logger.debug('[CMS] Fetching category tree', { tenantId: tenant.uuid, contentTypeUuid });
      return await categoryService.admin.getTree(contentTypeUuid);
    },
    enabled: !!tenant?.uuid && !!user?.id && userType === 'tenant',
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
...
export const usePublicCategoryTreeQuery = (contentTypeUuid?: string) => {
  return useQuery({
    queryKey: [...queryKeys.categories.tree(contentTypeUuid), 'public'],
    queryFn: async (): Promise<ApiResponse<CategoryTreeNode[]>> => {
      logger.debug('[CMS] Fetching public category tree', { contentTypeUuid });
      return await categoryService.public.getTree(contentTypeUuid);
    },
    enabled: true,
    staleTime: 10 * 60 * 1000,
    gcTime: 15 * 60 * 1000,
