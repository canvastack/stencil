...
export const ContentManagement: React.FC = () => {
  ...
  const loadPages = async () => {
    try {
      ...
      console.error('Failed to load tenant pages:', error);
      toast({
        title: 'Error',
        description: 'Failed to load business pages',
        variant: 'destructive'
      });
    } finally {
      setLoading(false);
  ...
  const handleDeletePage = async (slug: string, title: string) => {
    ...
    try {
      const response = await tenantApiClient.delete(`/tenant/content/pages/${slug}`);
  ...
  const handlePublishPage = async (slug: string, title: string) => {
    try {
      const response = await tenantApiClient.patch(`/tenant/content/pages/${slug}/publish`);
  ...
  const handleArchivePage = async (slug: string, title: string) => {
    try {
      const response = await tenantApiClient.patch(`/tenant/content/pages/${slug}/archive`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Business Content Management</h1>
          <p className="text-gray-600 mt-1">
            Manage your business website content and customer-facing pages
          </p>
          {tenant && (
      ...
      <Card>
        <CardHeader>
          <CardTitle>Business Pages</CardTitle>
          <CardDescription>
            Manage content for your business website including home, about, services, and contact pages
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 mb-4">
                  {searchTerm || statusFilter !== 'all' 
                    ...
                    : 'Get started by creating your first business page.'
                  }
                </p>
                {(!searchTerm && statusFilter === 'all') && (
                  <Link to="/admin/content/create">
                  ...
                  <div key={page.id} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <Link to={`/admin/content/edit/${page.slug}`}>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <Link to={`/admin/content/preview/${page.slug}`}>
                              <DropdownMenuItem>
